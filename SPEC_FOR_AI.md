...
```
src/
├── index.js      ★ メインエントリ。全インタラクションハンドラーcron管理
...
2. カレンダーのボタンを削除
3. `client.destroy()` して `logger.close()` → `process.exit(0)`
...
- [x] `clientReady` イベント対応（Discord.js v15 下互換性の確保）
- [x] `unhandledRejection` / `uncaughtException` ハンドラ（プロセスクラッシュ防止）
- [x] cron 多重実行防止（`isRunning` フラグ）
...
*この仕様書は 2026-05-17 時点のコードベースに基づいています。*
