...
```
src/
├── index.js        ★ メインエントリ。全インタラクションハンドラ・cron管理・グローバルコマンド登録
...
**v5 から引数に `guildId` が追加された。**

| 関数名 | 返り値 | 説明 |
|--------|--------|------|
| `buildCalendarEmbed(guildId, events, year, month)` | `EmbedBuilder` | カレンダー本体 |
| `buildCalendarButtons(year, month)` | `ActionRowBuilder` | ◀▶🔄 ボタン行 |
| `buildStatusEmbed(guildId, events, lastUpdated, roleName, online?)` | `EmbedBuilder` | ステータス（直近予定・件数等） |
...
| `formatEvent(event)` | イベントを表示文字列に整形 |
...
*この仕様書は 2026-05-26 時点のコードベースに基づいています。*
